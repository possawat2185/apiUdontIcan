...
router.get("/", (req, res) => {
  res.send("upload image deliveryJob work....");
});
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const uniqueFilename = `JOB${Date.now()}-${uuidv4()}}`;
      const storageRef = ref(storage, `images/jobs/${uniqueFilename}`);
      const metadata = {
      ...
      return res.status(200).json({
        message: "File uploaded deliveryJob successfully",
        fileName: uniqueFilename,
        url: downloadURL,
      });
    } catch (error: any) {
      console.error("Error uploading file deliveryJob:", error);
